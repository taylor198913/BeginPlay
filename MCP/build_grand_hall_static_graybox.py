...
def spawn(label: str, loc, scale, rot=(0, 0, 0), actor_type="StaticMeshActor"):
    name = actor_name(label)
    response = send("spawn_actor", {
        "name": name,
        "type": actor_type,
        "location": list(loc),
        "rotation": list(rot),
...
def build():
    ...
    for index, (x, y) in enumerate(column_positions, start=1):
        spawn(f"040_Column_{index:02d}", (x, y, 245), (0.45, 0.45, 5.3))
    ...
    spawn("054_RoundWindow_Block", (870, 0, 650), (0.18, 1.05, 1.05))
    spawn("055_LeftWall_Niche", (400, -610, 250), (1.2, 0.12, 2.4))
    spawn("056_RightWall_Niche", (400, 610, 250), (1.2, 0.12, 2.4))

    spawn("060_CenterPiano_Block", (-250, 0, 45), (1.5, 1.1, 0.45))
...
if __name__ == "__main__":
    build()
    created = [name for name in gh_names() if name.startswith(RUN_PREFIX)]
